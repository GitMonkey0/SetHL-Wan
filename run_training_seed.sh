...
codebook="$root/data_sources/wlasl/vq26_codebook.pt"
...
case "$method" in
  sethl)
    init="$root/runs/pretrain_sethl_aw1_seed${seed}.pt"
    ...
    python "$root/train_sethl_wan.py" "${train_common[@]}" \
      ...
      --output "$root/runs/wlasl_hand_cell_sethl_seed${seed}"
    ;;
  sethl_nohier)
    init="$root/runs/pretrain_sethl_aw0_seed${seed}.pt"
    [[ -f "$init" ]] || python "$root/pretrain_completion.py" "${common[@]}" \
      --representation sethl --angular-weight 0 --output "$init"
    python "$root/train_sethl_wan.py" "${train_common[@]}" \
      --representation sethl_nohier --angular-weight 0 --completion-init "$init" \
      --output "$root/runs/wlasl_hand_cell_no_spherical_seed${seed}"
    ;;
  hardhl)
    init="$root/runs/pretrain_sethl_aw1_seed${seed}.pt"
    [[ -f "$init" ]] || python "$root/pretrain_completion.py" "${common[@]}" \
    ...
    python "$root/train_sethl_wan.py" "${train_common[@]}" \
      ...
      --output "$root/runs/wlasl_hand_pre_hardhl_seed${seed}"
    ;;
  vq)
    init="$root/runs/pretrain_vq_aw1_seed${seed}.pt"
    [[ -f "$init" ]] || python "$root/pretrain_completion.py" "${common[@]}" \
    ...
    python "$root/train_sethl_wan.py" "${train_common[@]}" \
      ...
      --output "$root/runs/wlasl_hand_stable_vq_seed${seed}"
    ;;
  continuous)
    init="$root/runs/pretrain_continuous_seed${seed}.pt"
    [[ -f "$init" ]] || python "$root/pretrain_completion.py" "${common[@]}" \
      --representation continuous --output "$init"
    python "$root/train_sethl_wan.py" "${train_common[@]}" \
      --representation continuous --completion-init "$init" \
      --output "$root/runs/wlasl_hand_pre_continuous_seed${seed}"
    ;;
  raster)
    python "$root/train_sethl_wan.py" "${train_common[@]}" \
      --representation raster --output "$root/runs/wlasl_hand_raster_seed${seed}"
    ;;
