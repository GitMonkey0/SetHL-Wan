...
\begin{abstract}
Pose-conditioned video generators normally interpret control as a complete
trajectory.  This is unnecessarily rigid when an animator specifies only the
semantically important fingers or time intervals.  We formulate
\emph{partially specified hand control}: generated videos must obey named hand
constraints while retaining variation elsewhere.  We introduce SetHL, a
distribution-valued extension of Hand Labanotation (HL), and adapt a
Wan2.1-Fun-Control 1.3B model with rank-16 LoRA.  A masked transformer completes
unspecified symbols as distributions, and a certified in-cell sampler adds
continuous variation without changing their HL labels. A differentiable
hand-kinematic layer then maps them, without a joint-coordinate bypass, to
Wan's control latent.
Experiments on a quality-filtered WLASL subset compare fixed HL with matched
continuous Gaussian, learned-vector-quantized, and hard-symbol controls under
identical training budgets. We evaluate semantic compliance, diversity on
user-declared free cells, leakage, and temporal quality with paired
source-level bootstrap intervals. Across 27 test sources and three training
seeds, SetHL improves specified-symbol accuracy by 1.59 and 1.34 percentage
points over continuous and learned VQ controls, respectively, while reducing
leakage by 3.87 and 2.80 points; neither compliance--diversity hypervolume
difference is significant. On all 269 test clips, SetHL also reduces
conditional denoising MSE by \DenoiseRelative\% relative to continuous control. This turns
HL from a documentation format into an editable generative-video interface.
\end{abstract}
...
\label{sec:intro}
Hand motion is both semantically dense and difficult to synthesize.  Existing
controllers typically request every joint at every frame~\cite{controlnet,
sparsectrl,motioni2v,motionctrl}.  Such a signal is suitable for reenactment, but it is a poor
...
~\cite{motioni2v,motionctrl,wanmove,motionprompt}. Sparse 3-D hand control has
...
the generated-video benchmark uses the predeclared interval mask. We report specified-symbol accuracy, specified angular error,
...
matched 26-symbol controllers. Relative to the no-spherical-loss ablation, full SetHL
changes HV by \SphericalDelta{} (\SphericalCI), specified accuracy by
\SphericalAccDelta{} percentage points (\SphericalAccCI) and angular error by
\SphericalAngDelta$^\circ$ (\SphericalAngCI). Center-only inference gives
\CenterAUC{} HV; full SetHL differs by \CenterDelta{} (\CenterCI).
Thus spherical geometry shifts the trade-off toward semantic compliance rather
than uniformly increasing diversity, while within-cell residual sampling is
secondary at this resolution.
Hard HL reaches \HardAUC{} HV; full SetHL differs by \HardDelta{} (\HardCI),
showing that diffusion noise already supplies substantial free-cell variation
and that SetHL does not improve the aggregate frontier over hard symbols.
...
OpenAI Codex assisted with manuscript drafting, analysis code, and figure
scripting. The authors reviewed all outputs and take responsibility for the
paper's content and results.
